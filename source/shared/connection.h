        ...
        void close() {
            ...
            if (stream_.is_open() && status() == status_codes::ONLINE) {
                ...
                boost::asio::dispatch(write_strand_, [self]() {
                    self->stream_.async_close(boost::beast::websocket::close_code::normal,
                                              std::bind(&connection::close_handler,
            ...
            if (status() == status_t::ONLINE) {
                ...
                boost::asio::dispatch(read_strand_, [self]() {
                    self->stream_.async_read(
                        ...
                        boost::asio::bind_executor(
                            self->read_strand_,
                            std::bind(&connection::read_handler, self->shared_from_this(),
