...
public:
    ...
    struct server_session {
        ...
        server_session(iolet*, Ts&& ...args){
            session_ = std::make_shared<websocket_session_type>(std::forward<Ts>(args)...);
        }
...
private:
    ...
    bool is_ctx_running() const {
        return ctx_running.load();
    }
    ...
    template<typename T>
    c74::min::atoms attr_restrict_long(T lo, T hi, const c74::min::attribute<T>& attr, const c74::min::atoms& args){
    ...
    c74::min::atoms attr_check_ip(const c74::min::attribute<c74::min::symbol>& attr, const c74::min::atoms& args){
